	...
	private void setzeFigur(int x1, int y1, int x2, int y2)
	{
		...
		if(f instanceof Dummy == false)
		{
			if(f instanceof Bauer)
				...
				geschlagen.add("Turm " + (f.isBlack() ? "Schwarz" : "Weiß"));
			else if(f instanceof Springer)
				geschlagen.add("Springer " + (f.isBlack() ? "Schwarz" : "Weiß"));
			else if(f instanceof Laeufer)
				geschlagen.add("Laeufer " + (f.isBlack() ? "Schwarz" : "Weiß"));
			else if(f instanceof Dame)
				geschlagen.add("Dame " + (f.isBlack() ? "Schwarz" : "Weiß"));
		}
